...
description: |
  ...
  1. Distill mode: Extract a design system from a website URL (via dembrandt) or local project —
     ...
     Saves to both project (`./DESIGN.md`) and global library (`~/.config/design-distill/<name>/DESIGN.md`).
...
Before any distill operation, ensure dembrandt is installed:
...
- **Not found** → install it:
  ...
  ```
  If installation fails (no npm, permission denied, network error), warn the user and continue — the skill will fall back to browser-based extraction.

After dembrandt is confirmed, check Playwright browser availability:
...
- **Not found** → install it:
  ...
  ```
  If installation fails, warn: "Playwright browsers could not be installed. Falling back to browser-based extraction."
...
```bash
find . -maxdepth 3 -name "DESIGN.md" 2>/dev/null | head -5
```

**Resolution order** (first match wins):

1. **Explicit named style** (from user input) → load from `~/.config/design-distill/<name>/DESIGN.md`, enter **Design Mode**
2. **Project `DESIGN.md`** found → enter **Design Mode**
3. **No file, no named style** → enter **Distill Mode**

---

## Global Style Library

Design systems are stored in a global library at `~/.config/design-distill/`. Each style is a directory (kebab-case) containing a `DESIGN.md` file.
...
```bash
ls -d ~/.config/design-distill/*/ 2>/dev/null
```

For each directory, read the `source_url` and `distilled` fields from its `DESIGN.md` header and display:
...
```bash
cat ~/.config/design-distill/<name>/DESIGN.md 2>/dev/null
```
...
If dembrandt + Playwright were set up successfully (see Setup), use dembrandt. Otherwise fall back to `/browse`.

##### With dembrandt (default)
...
# (write ~/.config/design-distill/<name>/DESIGN.md with identical content)
```
